...
class NodeUpdateNetwork(nn.Module):
    def __init__(self,
        ...
        for l in range(len(self.num_features_list)):
            ...
            if self.dropout > 0:#(len(self.num_features_list) - 1)and l == 0
                layer_list['drop{}'.format(l)] = nn.Dropout2d(p=self.dropout)

        self.network = nn.Sequential(layer_list)

    def forward(self, node_feat, edge_feat, l):
        # get size
        num_tasks = node_feat.size(0)
        num_data = node_feat.size(1)
        node_feat = node_feat.view(1, num_tasks * num_data, node_feat.size(2))
...
class GraphNetwork(nn.Module):
    def __init__(self, args):
        ...
        for l in range(self.num_layers):
            ...
            edge2node_net = NodeUpdateNetwork(
                ...
                num_features=self.node_features if l != self.num_layers - 1 else args.num_class,
                dropout=self.dropout if l < self.num_layers - 1 else 0.0)
    ...
    # forward
    def forward(self, src_feat, tar_feat, init_edge_feat, target_mask):
        # for each layer
        edge_feat_list = []
        node_feat_list = []
        edge_feat = init_edge_feat

        node_feat = torch.cat([src_feat, tar_feat], axis=1)
        pre_node_list = []
        for l in range(self.num_layers):
            # (1) edge update
            ...
            # (2) node update
            node_feat = self._modules['edge2node_net{}'.format(l)](node_feat, edge_feat, l)
